...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if remindSwitch.isOn {
            bill.scheduleReminder(dateScheduled: remindDatePicker.date) { (updatedBill) in
                if let _ = updatedBill.notificationID {
    ...
    func presentNeedAuthorizationAlert() {
        let alert = UIAlertController( title: "Reminders Unavailable", message: "Reminders are unavailable unless granted notification permission in iOS Settings", preferredStyle: .alert )
